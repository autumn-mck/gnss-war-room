...
As it may be desirable to display the data in a range of other locations, the system should additionally provide a web interface, allowing users to view the data in a browser. The system must be flexible to running in a range of environments, eg. all running on the same system, or split across multiple.

The system should be as easy as possible to set up and run, and include instructions for doing so.
...
- A window displays the location of satellites over a map of the earth
- In the map of the earth, the user can move around the point being focused on
- In the map of the earth, the zoom level is configurable
- Which network a satellite is part of is indicated by a unique colour
- Map should have a key to indicate which colour represents each network
- Optionally, map displays ADS-B (Automatic Dependent Surveillance-Broadcast, for aircraft) data
- Optionally, map displays AIS (Automatic identification system, for marine vessels) data
- A window displays the elevation and azimuth and elevation of each satellite (e.g. on a polar grid)
- The system should allow for multiple windows of the same type, e.g. two maps focused on different points
- A window displays signal strength for each satellite
- A window showing raw GNSS NMEA messages
- A window showing statistics derived from GNSS messages (Latitude/longitude, time, altitude) along with information on the signal quality (dilution of precision, fix quality)
- A window displaying a comparison between GNSS timing and the precise, accurate time source
- This window displays additional statistics derived from data (Allan variance, etc.)
- Application allows multiple colour schemes to be selected from
...
- Application should be configurable through a configuration file in a human readable format, e.g. (no need to change the code)
...
Changes will be tracked with git, and the project will be hosted on the EEECS GitLab.
...
- Converting the azimuth and elevation data of satellites from the hemisphere the GNSS receiver sees to latitude and longitude was significantly more challenging than expected. I was able to verify that my solution appeared to be correct by creating visualisations using existing graphing software, and where the final points appeared when plotted on a map.
- Finding a suitable font for the UI was a challenge, as the font used in the original WarGames did not appear to be available anywhere. I was able to get a close enough match by using the vector font for the HP1345A display, the displays used to capture the images in the original WarGames.
...
- Had not anticipated other modules taking as much time as they did, meaning I had to take a complete break from the project at the start of December. However, more work than anticipated was able to be completed over Christmas to make up for this.
- Original plan/problem description had not included MQTT. Adding it did not effect the overall development of the project.
- Original plan had not included web interface. Adding it did not effect the overall development of the project.
