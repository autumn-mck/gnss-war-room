...
class RawMessageWindow(QMainWindow):
	...
	def __init__(self, palette: Palette, config: RawMessageConfig):
		...
		self.svgFont = Font()
		_, charWidth, charHeight = makeTextGroup(self.svgFont,
			"$".encode('ascii'),
	...
	def onNewData(self, message: bytes):
		...
		newGroup, _, _ = makeTextGroup(self.svgFont,
			message,
			scale=self.textScale,
			fontThickness=self.config.fontThickness,
	...
	def updateMessageLog(self):
		...
		for i, messageSvgGroup in enumerate(self.messageSvgGroups):
			strToAdd = f'<g transform="translate(0, {i * heightOfMessage})" stroke-linecap="round" stroke-linejoin="round" fill="none">{messageSvgGroup}</g>'
		...
		svgToDisplay = f'<svg version="1.1" viewBox="0 0 {totalWidth} {heightOfMessage * len(self.messageSvgGroups)}">{svgToDisplay}</svg>'
